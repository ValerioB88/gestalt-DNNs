...
class SaveInfoCsv(Callback):
    def __init__(self, log_names, path, update_on_batch_end=False, update_on_epoch_end=True, update_on_train_end=False, append=False):
        ...
        f = open(self.path, 'wa' if append else 'w')
        csv_writer = writer(f)
...
class PrintLogs(Callback, ABC):
    def __init__(self, id, plot_every=100, log_prefix='', plot_at_end=True):
        self.id = id
        self.last_iter = 0
