...
def collect(network_name, pretraining, background):
    ...
    cc = {k: i[ll] for k, i in cs.items()}
    CE = np.array(cc['empty-single']) - np.array(cc['empty'])

    return {'mean': -np.mean(CE), 'std': np.std(CE)}
...
[plt.bar(idx, a[nn]['mean'],
              yerr=a[nn]['std']) for idx, nn in enumerate(network_names)]
